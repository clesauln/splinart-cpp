...
///
inline void draw_pixel(xt::xtensor<double, 3> &img,
                       const xt::xtensor<double, 2> &spline,
                       xt::xtensor_fixed<double, xt::xshape<4>> color) {
...
///
inline void update_path(xt::xtensor<double, 2> &path,
                        double scale_value = 0.00001, bool periodic = false) {
  auto n = path.shape(0);
...
inline auto build_img(const std::array<std::size_t, 2> &img_size,
                      std::vector<Circle> &circles, std::size_t rep = 300,
                      bool periodic = true, double scale_color = 0.005,
  ...
  auto xs_func = []() {
    ...
    return xt::eval(xt::fmod(xt::random::rand<double>({1})[0] +
                                 xt::linspace<double>(0, 2 * pi, nsamples),
                             2 * pi));
  };
